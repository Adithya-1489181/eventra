...
import { Button, Input, Textarea, Card, Alert, Select, Spinner } from '../components/ui';
...
const EditEvent = () => {
  ...
  const fetchEventDetails = async () => {
    try {
      ...
      const event = transformEventData(rawEvent);

      // Parse dates
      const startDate = event.startDate ? new Date(event.startDate) : null;
      const endDate = event.endDate ? new Date(event.endDate) : null;

      // Determine venue type from location object
      let venueType = 'venue';
      let locationValue = '';
      if (typeof rawEvent.location === 'object' && rawEvent.location) {
        venueType = rawEvent.location.type || 'venue';
        locationValue = rawEvent.location.venue || '';
      } else {
        locationValue = rawEvent.location || '';
      }
      ...
      setFormData({
        ...
        ticketPrice: rawEvent.ticket_price?.toString() || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Build location object based on venue type
      const locationObj = {
        type: formData.venueType,
        venue: formData.venueType === 'venue' ? formData.location : 'Online Event'
      };
      ...
      await apiClient.patch(`/api/events/${eventId}`, eventDetails);
